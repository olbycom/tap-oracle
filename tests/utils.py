...
def get_test_connection():
    ...
    conn_string = "(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST={})(PORT={}))(CONNECT_DATA=(SID={})))".format(
        creds["host"], creds["port"], creds["sid"]
    )
    conn_config = {"user": creds["user"], "password": creds["password"], "dsn": conn_string}
...
def ensure_test_table(table_spec):
    ...
    with get_test_connection() as conn:
        ...
        old_table = cur.execute(
            "select * from all_tables where owner  = '{}' AND table_name = '{}'".format("ROOT", table_spec["name"])
        ).fetchall()
        if len(old_table) != 0:
...
def insert_record(cursor, table_name, data):
    ...
    insert_sql = """ INSERT INTO {}
                            ( {} )
                     VALUES ( {} )""".format(
        table_name, columns_sql, value_sql
    )
