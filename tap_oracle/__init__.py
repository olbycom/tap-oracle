...
FLOAT_TYPES = set(["binary_float", "binary_double"])
...
def schema_for_column(c, pks_for_table, use_singer_decimal, date_as_string=False):
    ...
    # Check for Oracle's XMLTYPE
        ...
        result.type = nullable_column(c.column_name, "string", pks_for_table)
        result.maxLength = (
            4294967295  # Technically 4GB -1 https://docs.oracle.com/cd/E18283_01/server.112/e17110/limits001.htm
        )
...
def produce_column_metadata(
    connection, database_name, table_info, table_schema, table_name, pk_constraints, column_schemas, cols
):
    mdata = {}
...
def discover_columns(connection, table_info, filter_schemas, filter_tables, use_singer_decimal, date_as_string=False):
    ...
    if filter_tables:
        ...
        table_filter = """owner||'-'||table_name IN ({})""".format(
            ",".join(f"'{t.rstrip('.*')}'" for t in filter_tables)
        )
    ...
    for k, cols in itertools.groupby(columns, lambda c: (c.table_schema, c.table_name)):
        ...
        LOGGER.info(f"--- DIAGNOSTIC: Schema Comparison for {table_schema}.{table_name} ---")
        for c in cols:
...
def do_discovery(conn_config, filter_schemas, filter_tables, use_singer_decimal, date_as_string=False):
   ...
   WHERE {filter}""",
    ...
    if filter_tables:
        ...
        sql = sql + """ AND owner||'-'||table_name IN ({})""".format(
            ",".join(f"'{t.rstrip('.*')}'" for t in filter_tables)
        )
    ...
    for row in cur.execute(sql, filter_schemas):
        ...
        is_view = False
        table_info[schema][table] = {"row_count": row_counts[table], "is_view": is_view}
   ...
   WHERE {filter}""",
    ...
    if filter_tables:
        ...
        sql = sql + """ AND owner||'-'||view_name IN ({})""".format(
            ",".join(f"'{t.rstrip('.*')}'" for t in filter_tables)
        )
...
def do_sync_incremental(conn_config, stream, state, desired_columns, date_as_string=False, date_format="YYYY-MM-DD"):
    ...
    LOGGER.info(
        "Stream %s is using incremental replication with replication key %s", stream.tap_stream_id, replication_key
    )
...
def sync_method_for_streams(streams, state, default_replication_method):
    ...
    for stream in streams:
        ...
        if replication_method == "LOG_BASED" and stream_metadata.get((), {}).get("is-view"):
            raise Exception(
                "LogMiner is NOT supported for views. Please change the replication method for {}".format(
        ...
        if replication_method == "FULL_TABLE":
            ...
            elif get_bookmark(state, stream.tap_stream_id, "ORA_ROWSCN") and get_bookmark(
                state, stream.tap_stream_id, "scn"
            ):
            ...
            # inconsistent state
            elif get_bookmark(state, stream.tap_stream_id, "ORA_ROWSCN") and not get_bookmark(
                state, stream.tap_stream_id, "scn"
            ):
                raise Exception(
...
def sync_log_miner_streams(conn_config, log_miner_streams, state, end_scn, date_as_string=False):
    if log_miner_streams:
        log_miner_streams = list(map(log_miner.add_automatic_properties, log_miner_streams))
        state = log_miner.sync_tables(conn_config, log_miner_streams, state, end_scn, date_as_string)
...
def sync_traditional_stream(conn_config, stream, state, sync_method, end_scn, date_as_string=False, date_format="YYYY-MM-DD"):
    ...
    if sync_method == "full":
        ...
        if md_map.get((), {}).get("is-view"):
            state = full_table.sync_view(conn_config, stream, state, desired_columns, date_as_string)
        else:
            state = full_table.sync_table(conn_config, stream, state, desired_columns, date_as_string)
