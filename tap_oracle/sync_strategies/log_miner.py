...
def get_connection_with_common_user_or_default(conn_config):
    ...
    if (
        conn_config.get("common_user")
        and conn_config.get("common_password")
        and (conn_config.get("common_service_name") or conn_config.get("common_sid"))
    ):
...
def add_automatic_properties(stream):
    ...
    stream.schema.properties["_sdc_deleted_at"] = Schema(type=["null", "string"], format="date-time")
...
def row_to_singer_message(stream, row, version, columns, time_extracted):
    ...
    return singer.RecordMessage(stream=stream.tap_stream_id, record=rec, version=version, time_extracted=time_extracted)
...
def sync_tables_logminer(cur, streams, state, start_scn, end_scn):
    ...
    for stream in streams:
        ...
        redo_value_sql_clause = ",\n ".join(
            ["""DBMS_LOGMNR.MINE_VALUE(REDO_VALUE, :{})""".format(idx + 1) for idx, c in enumerate(desired_columns)]
        )
        undo_value_sql_clause = ",\n ".join(
            ["""DBMS_LOGMNR.MINE_VALUE(UNDO_VALUE, :{})""".format(idx + 1) for idx, c in enumerate(desired_columns)]
        )
      ...
      """.format(
            redo_value_sql_clause, undo_value_sql_clause
        )
        binds = (
            ...
            for op, redo, scn, cscn, commit_ts, *col_vals in cur.execute(mine_sql, binds):
                ...
                if op == "INSERT" or op == "UPDATE":
                    ...
                    record_message = row_to_singer_message(
                        stream, redo_vals, stream_version, columns_for_record, time_extracted
                    )
                elif op == "DELETE":
                    undo_vals += [cscn, singer.utils.strftime(commit_ts.replace(tzinfo=pytz.UTC))]
                    record_message = row_to_singer_message(
                        stream, undo_vals, stream_version, columns_for_record, time_extracted
                    )
                else:
                ...
                counter.increment()
                state = singer.write_bookmark(state, stream.tap_stream_id, "scn", int(cscn))
